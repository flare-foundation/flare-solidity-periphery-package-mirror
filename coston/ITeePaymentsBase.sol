...
interface ITeePaymentsBase is ITeePaymentsModel {
    ...
    ) external payable returns (uint64 _paymentId);
    ...
    function reissue(
        ...
        bool _startNew,
        address _claimBackAddress
    ) external payable returns (bool _finalized);
    ...
    function getWalletAccounts(
        bytes32 _walletId
    ) external view returns (PMWMultisigAccount[] memory _walletAccounts);
    ...
    function getWalletId(
        PMWMultisigAccount calldata _account
    ) external view returns (bytes32 _walletId);
    ...
    function getAuthorizationAddress(
        PMWMultisigAccount calldata _account
    ) external view returns (address _authorizationAddress);
    ...
    function getPaymentHash(
        PMWMultisigAccount calldata _account,
        uint64 _paymentId
    ) external view returns (bytes32 _paymentHash);
    ...
    function getNextPaymentId(
        PMWMultisigAccount calldata _account
    ) external view returns (uint64 _nextPaymentId);
}

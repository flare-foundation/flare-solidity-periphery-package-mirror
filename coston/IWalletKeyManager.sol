...
interface IWalletKeyManager is ITeeCommonErrors {
    ...
    event WalletKeysNotAvailable(bytes32 indexed walletId, uint64[] keyIds);
    ...
    ) external payable returns (uint64 _keyId);
    ...
    function receivingTeesAndKeys(
        bytes32 _walletId
    ) external returns (TeeIdKeyIdPair[] memory _teeIdKeyIdPairs);
    ...
    function getReceivingTeeIds(
        bytes32 _walletId
    ) external view returns (address[] memory _teeIds);
    ...
    function getWalletKeyPublicKey(
        bytes32 _walletId,
        uint64 _keyId
    ) external view returns (bytes memory _publicKey);
    ...
    function getKeyNonce(
        ...
        bytes32 _walletId,
        uint64 _keyId
    ) external view returns (uint256 _nonce, bool _teeHoldsKey);
    ...
    function getWalletPublicKeys(
        ...
        external
        view
        returns (uint64 _multisigThreshold, bytes[] memory _publicKeys);
}

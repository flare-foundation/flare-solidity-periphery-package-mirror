...
interface ITeePaymentsRegistry {
    ...
    event SourcesRegistered(SourceRegistration[] registrations);

    event SourcesUnregistered(bytes32[] sourceIds);
    ...
    error WrongPaymentModel(
        address teePayments,
        PaymentModel expected,
        PaymentModel actual
    );
    ...
    function getTeePaymentsForSource(
        bytes32 _sourceId
    ) external view returns (address _teePayments);
    ...
    function getSourceConfig(
        bytes32 _sourceId
    ) external view returns (SourceConfig memory _sourceConfig);
    ...
    function getSourceOpTypeAndTeePayments(
        bytes32 _sourceId
    ) external view returns (bytes32 _opType, address _teePayments);
    ...
    function getSourceKeyTypeAndTeePayments(
        bytes32 _sourceId
    ) external view returns (bytes32 _keyType, address _teePayments);
    ...
    function getSourcePaymentModel(
        bytes32 _sourceId
    ) external view returns (PaymentModel _paymentModel);
    ...
    ) external view returns (bytes32[] memory _sourceIds);
     ...
     */
    function isSourceRegistered(bytes32 _sourceId) external view returns (bool);
}

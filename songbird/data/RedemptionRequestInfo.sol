...
library RedemptionRequestInfo {
    ...
    struct DataExt {
        ...
        // The fee in NAT that the executor receives if they successfully call default.
        uint256 executorFeeNatWei;
        // True if the redemption requires a destination tag for the payment.
        // Must be requested with `redeemWithTag` and confirmed with `confirmXRPRedemptionPayment`.
        bool requiresDestinationTag;
        // The destination tag required for the redemption payment, if `requiresDestinationTag` is true.

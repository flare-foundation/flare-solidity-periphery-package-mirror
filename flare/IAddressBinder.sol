...
interface IAddressBinder {
    ...
    function registerPublicKey(
        bytes calldata _publicKey
    ) external returns (bytes20 _pAddress, address _cAddress);
     ...
     **/
    function pAddressToCAddress(
        bytes20 _pAddress
    ) external view returns (address _cAddress);
     ...
     **/
    function cAddressToPAddress(
        address _cAddress
    ) external view returns (bytes20 _pAddress);
}

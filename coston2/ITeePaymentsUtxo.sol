...
import {IPMWMultisigUtxoConfigured} from "./IPMWMultisigUtxoConfigured.sol";
...
interface ITeePaymentsUtxo is ITeePaymentsBase {
    ...
    function getBatchSettings(
        PMWMultisigAccount calldata _account
    ) external view returns (uint64 _batchSize, uint64 _batchDurationSeconds);
    ...
    function getMaxBatchSettings(
        ...
        external
        view
        returns (uint64 _maxBatchSize, uint64 _maxBatchDurationSeconds);
    ...
    function getAnchorReuseDelay(
        bytes32 _sourceId
    ) external view returns (uint64 _anchorReuseDelaySeconds);
    ...
    ) external view returns (UtxoAnchorState memory _anchor);
    ...
    function getAnchorCount(
        PMWMultisigAccount calldata _account
    ) external view returns (uint256 _anchorCount);
    ...
    ) external view returns (BatchRecord memory _batch, bool _open);
    ...
    function getBatchPaymentId(
        PMWMultisigAccount calldata _account,
        uint64 _paymentId
    ) external view returns (uint64 _batchPaymentId);
}

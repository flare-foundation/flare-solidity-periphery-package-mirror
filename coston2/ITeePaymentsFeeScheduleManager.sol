...
interface ITeePaymentsFeeScheduleManager {
    ...
    event FeeScheduleConfigsSet(FeeScheduleConfigInput[] configs);

    event FeeScheduleConfigsCleared(bytes32[] sourceIds);
    ...
    error InvalidFeeScheduleConfig(
        bytes32 sourceId,
        uint8 maxSchedules,
        uint16 maxDelaySeconds
    );
    ...
    ) external view returns (bytes memory _feeSchedule);
    ...
    ) external view returns (bytes[] memory _encodedPerPayment);
    ...
    function getProjectFeeSchedule(
        bytes32 _projectId,
        bytes32 _sourceId
    ) external view returns (FeeSchedule[] memory _schedule);
    ...
    function getFeeScheduleConfig(
        bytes32 _sourceId
    ) external view returns (FeeScheduleConfig memory _config);
}

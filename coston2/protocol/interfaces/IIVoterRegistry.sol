...
interface IIVoterRegistry is IVoterRegistry {
     ...
     */
    function setNewSigningPolicyInitializationStartBlockNumber(
     ...
     * @dev Only FlareSystemsManager contract can call this method.
     */
    function createSigningPolicySnapshot(
        uint256 _rewardEpochId
    )
        external
        returns (
    ...
    function getRegisteredDelegationAddresses(
        uint256 _rewardEpochId
    ) external view returns (address[] memory _delegationAddresses);
    ...
    function getRegisteredSubmitSignaturesAddresses(
        uint256 _rewardEpochId
    ) external view returns (address[] memory _signingPolicyAddresses);
    ...
    function getRegisteredNodeIds(
        uint256 _rewardEpochId
    ) external view returns (bytes20[][] memory _nodeIds);
    ...
    function getRegisteredVotersAndNormalisedWeights(
        ...
        external
        view
        returns (address[] memory _voters, uint16[] memory _normalisedWeights);
    ...
    ) external view returns (address _voter, uint16 _normalisedWeight);

...
interface IWalletManager is ITeeCommonErrors {
    ...
    event WalletCreated(bytes32 indexed projectId, bytes32 indexed walletId);
    ...
    event WalletAdminConfirmed(bytes32 indexed walletId, address indexed admin);
    ...
    event WalletInitialized(bytes32 indexed walletId);

    event WalletEnabled(bytes32 indexed walletId);
    ...
    function createWallet(
        bytes32 _projectId
    ) external returns (bytes32 _walletId);
    ...
    function getProjectWalletIds(
        bytes32 _projectId
    ) external view returns (bytes32[] memory _walletIds);
    ...
    function getWalletProjectId(
        bytes32 _walletId
    ) external view returns (bytes32 _projectId);
    ...
    function getWalletAdminsAndThreshold(
        bytes32 _walletId
    ) external view returns (address[] memory _admins, uint64 _adminsThreshold);
    ...
    function getWalletStatus(
        bytes32 _walletId
    ) external view returns (WalletStatus _status);
}
